...
import com.eigengo.lift.Exercise.{Exercise, RequestedClassification}
...
class UserExercisesTracingTest extends FlatSpec with TestKitBase with Matchers with ImplicitSender {
  ...
  "The session events" should "be saved" in {
    ...
    val props = SessionProperties(new Date(), Seq(), 1.0, RequestedClassification.ExplicitClassification)
